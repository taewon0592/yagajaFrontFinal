...
public class PagingUtil {
	...
	public static String pagingImg(int totalRecordCount, int pageSize, int blockPage, int nowPage, String page) {
		...
		int blockCount = 1; // 페이지 표시 제어를 위한 변수
		// 4 페이지를 뿌려주는 로직 : blockPage의 수 만큼. 또는 마지막 페이지가 될 때 까지 페이지를 출력
	...
	public static String pagingImgServlet(int totalRecordCount, int pageSize, int blockPage, int nowPage, String page) {
		String pagingStr = "";
		// 1 전체페이지 구하기
		int totalPage = (int) (Math.ceil(((double) totalRecordCount / pageSize)));
		...
		// 처음페이지 & 이전페이지를 위한 로직
		if (intTemp != 1) {
			pagingStr += "<li>" + "<a href='" + page + "nowPage=1'>"
		...
		// 페이지표시 제어를 위한 변수
		int blockCount = 1;
		// 페이지를 뿌려주는 로직 : 블럭페이지 수만큼 혹은 마지막 페이지가 될때까지 페이지를 표시
		while (blockCount <= blockPage && intTemp <= totalPage) {
			// 현제페이지인 경우
			if (intTemp == nowPage) {
	...
	public static String pagingAjax(int totalRecordCount, int pageSize, int blockPage, int nowPage, String page) {
		...
		// 처음페이지 & 이전페이지를 위한 로직
		if (intTemp != 1) {
			pagingStr += "<li>" + "<a href='javascript:paging(1);'>"
					+ "<span class='glyphicon glyphicon-fast-backward'></span></a></li>";
			// pagingStr += "&nbsp;";
