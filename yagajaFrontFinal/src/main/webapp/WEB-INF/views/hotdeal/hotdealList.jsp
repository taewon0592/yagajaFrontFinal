...
function hotdeal_reverse_time_${row.hotdeal_no}()
{
	...
	dateVal	=	parseInt(remain_time_sec_${row.hotdeal_no}/(24*60*60));	//핫딜시작까지 남은 총 초에서 날짜 뽑기
	temp_val = remain_time_sec_${row.hotdeal_no}%(24*60*60);				//핫딜시작까지 남은 초 담기
	hourVal	= parseInt(temp_val/(60*60));				//핫딜시작까지 남은 총 초에서 시간 뽑기
	temp_val = temp_val%(60*60);						//핫딜시작까지 남은 초 담기
	minVal = parseInt(temp_val/(60));					//핫딜시작까지 남은 총 초에서 분 뽑기
	secVal = temp_val%(60);								//핫딜시작까지 남은 초 담기
	
	srts_dateVal = parseInt(srts_${row.hotdeal_no}/(24*60*60));	//핫딜시간 남은 총 초에서 날짜 뽑기
	srts_temp_val = srts_${row.hotdeal_no}%(24*60*60);			//핫딜시간 남은 초 담기
	...
	document.getElementById("diffTime_${row.hotdeal_no}").innerHTML = "<p style='color:blue; font-weight:bold'><i class='far fa-clock'></i> 핫딜까지 남은시간 : "+dateVal+"일 "+hourVal+"시간 "+minVal+"분 "+secVal+"초</p>";

	//남은 시간이 0이 되어 핫딜종료시
	if(remain_time_sec_${row.hotdeal_no}<=0)
	{
		document.getElementById("diffTime_${row.hotdeal_no}").innerHTML = "<p><i class='far fa-clock'></i> 핫딜 진행중 : "+srts_dateVal+"일 "+srts_hourVal+"시간 "+srts_minVal+"분 "+srts_secVal+"초</p>";
		//호출중지
		//화면리플레쉬
 
		if(srts_${row.hotdeal_no}<=0)
		{
			document.getElementById("diffTime_${row.hotdeal_no }").innerHTML = "<p style='color:red; font-weight:bold;'>입찰이 종료되었습니다</p>";
		}
		else
		{
...
{	
	...
	if(remain_time_sec_${row.hotdeal_no} > 0)
	{
		...
		//hotdeal_reverse_time_${row.hotdeal_no}(call_ref_hotdeal);
	}
	else
	{
            ...
            <div class="row">
                ...
                <div class="col-12 col-sm-6 col-lg-4">
                    ...
                    <c:choose>
                    	...
                    	<c:otherwise>
                    		<a href="../hotdeal/hotdealView.do?room_no=${row.room_no }&lodge_no=${row.lodge_no}&member_no=${sessionScope.siteUserInfo.member_no}&hotdeal_no=${row.hotdeal_no}">
                        		<img style="height:307px; width:100%;" src="../resources/common/img/room-img/${row.lodge_photo }" alt="">
                    		</a>		
                    	</c:otherwise>
                    </c:choose>
                        ...
                        <c:choose>
                          	<c:when test="${row.remain_time_sec >= 0 }">
                           		<div class="price-start2" style="font-weight:bold; font-size:17px;"id="diffTime_${row.hotdeal_no }">
                   					<p style="color:blue; font-weight:bold;"><i class="far fa-clock"></i> 핫딜까지 남은시간 : 0일 00시간 00분 00초</p> 
                   				</div>
                          	</c:when>
                          	<c:otherwise>
                          		<div class="price-start2" style="font-weight:bold; font-size:17px;"id="diffTime_${row.hotdeal_no }">
                   					<p style="color:rgb(255,52,121); font-weight:bold;"><i class="far fa-clock"></i> 핫딜 진행중 : 0일 00시간 00분 00초</p> 
                   				</div>
                          	</c:otherwise>			
                        </c:choose>			
                        <div class="feature-content d-flex align-items-center justify-content-between">
                            <div class="row feature-title">
                            	...
                            	<div class="col-7 col-lg-7" style="font-weight:bold; font-size:15px; ">
                            		<span style="color:gray">핫딜 특가 :</span>&nbsp;
                            		<span style="color:red; font-size:17px">
                         				<fmt:formatNumber value="${row.hotdeal_price }" groupingUsed="true"/>원</span>
                            	</div>
                            	<div class="col-12 col-lg-12" style="font-weight:bold; font-size:17px; ">
                            		<span style="color:rgb(255,52,121)">핫딜 남은 수량 :</span>
                            		<span style="color:rgb(255,52,121)">${row.hotdeal_sell }개 </span>
                            	</div>
                            	<div class="col-12 col-lg-12" style="font-weight:bold; font-size:17px; ">
                            		사용가능일 : <span>${row.hotdeal_check_in}</span>
                            	</div>
                            	<div class="col-12 col-lg-12" style="font-weight:bold; font-size:17px; ">
                            		사용마감일 : <span>${row.hotdeal_check_out }</span>
                            	</div>
                              </div>
		                   </div>
                    </div>
